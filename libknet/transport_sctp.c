...
static void _lock_sleep_relock(knet_handle_t knet_h)
{
	...
	while (i < 5) {
		// DEBUG
		usleep(knet_h->threads_timer_res / 8);
		...
		case SCTP_CONNECT_LINK_INFO:
			...
			if (recv_err != 1) {
				if (connect_info->on_rx_epoll) {
					memset(&ev, 0, sizeof(struct epoll_event));
					ev.events = EPOLLIN;
					ev.data.fd = connect_info->connect_sock;
					if (epoll_ctl(knet_h->recv_from_links_epollfd, EPOLL_CTL_DEL, connect_info->connect_sock, &ev)) {
						log_err(knet_h, KNET_SUB_TRANSP_SCTP, "Unable to remove connected socket from epoll pool: %s",
						strerror(errno));
						return -1;
					}
...
int sctp_transport_link_is_down(knet_handle_t knet_h, struct knet_link *kn_link)
{
	...
	sctp_handle_info_t *handle_info = knet_h->transports[KNET_TRANSPORT_SCTP];
	sctp_connect_link_info_t *info = kn_link->transport_link;
	sctp_accepted_link_info_t *accepted_info = knet_h->knet_transport_fd_tracker[kn_link->outsock].data;
	sctp_listen_link_info_t *listen_info;

	if (kn_link->outsock < 0) {
		return 0;
	}
	...
	if (kn_link->dynamic == KNET_LINK_STATIC) {
		...
		if (listen_info->on_rx_epoll) {
			memset(&ev, 0, sizeof(struct epoll_event));
			ev.events = EPOLLIN;
			ev.data.fd = kn_link->outsock;
			if (epoll_ctl(knet_h->recv_from_links_epollfd, EPOLL_CTL_DEL, kn_link->outsock, &ev)) {
				log_err(knet_h, KNET_SUB_TRANSP_SCTP, "Unable to remove EOFed socket from epoll pool: %s",
				strerror(errno));
				return -1;
			}
		...
		log_debug(knet_h, KNET_SUB_TRANSP_SCTP, "Notifying listen thread that sockfd %d received an error", kn_link->outsock);
		if (sendto(handle_info->listensockfd[1], &kn_link->outsock, sizeof(int), MSG_DONTWAIT | MSG_NOSIGNAL, NULL, 0) != sizeof(int)) {
			log_debug(knet_h, KNET_SUB_TRANSP_SCTP, "Unable to notify listen thread: %s", strerror(errno));
		}
	}
...
static void _handle_listen_sctp_errors(knet_handle_t knet_h)
{
	...
	if (_is_valid_fd(knet_h, sockfd) < 1) {
		log_debug(knet_h, KNET_SUB_TRANSP_SCTP, "Received stray notification for listen socket fd error %d", sockfd);
		return;
	}
